...
__attribute__((always_inline)) inline void fm_synth_get_samples(uint16_t output[], uint16_t startFrame, uint16_t endFrame)
...
//	for (uint16_t noteIndex = 0; noteIndex < (nkeys); noteIndex++)
	{
		...
		for (uint16_t iFrame = startFrame; iFrame < endFrame; iFrame+=2) {
			...
			if (note.insFMda > 0) {
				if (note.FMamp > (uint32_t)note.insFM_ampl_end)
